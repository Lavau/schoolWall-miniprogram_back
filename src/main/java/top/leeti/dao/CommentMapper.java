...
public interface CommentMapper {
    ...
    @Select("SELECT _id AS id, _promulgator_id AS promulgatorId, _attached_id As attachedId, _parent_id AS parentId, " +
            ...
            "_comment._is_available AS Available, _comment._is_audit AS Audit FROM _comment LEFT JOIN _user ON " +
            "_user._stu_id = _comment._promulgator_id WHERE _comment._is_available = 1 AND _comment._is_audit = 1 " +
            "AND _attached_id = #{attachedId} ORDER BY _comment._gmt_create DESC")
    ...
    @Select("SELECT _id AS id, _promulgator_id AS promulgatorId, _attached_id As attachedId, _parent_id AS parentId, " +
            ...
            "_comment._is_available AS Available, _comment._is_audit AS Audit FROM _comment LEFT JOIN _user ON " +
            "_user._stu_id = _comment._promulgator_id WHERE _comment._is_available = 1 AND _comment._is_audit = 1 " +
            "AND _parent_id = #{parentId} ORDER BY _comment._gmt_create ASC")
    List<Comment> listCommentsOfRepliedComment(@Param("parentId") String parentId);

    @Select("SELECT count(_id) FROM _comment WHERE _is_available = 1 AND _is_audit = 1 AND _parent_id = #{parentId}")
    ...
    @Select("SELECT _id AS id, _promulgator_id AS promulgatorId, _attached_id As attachedId, _parent_id AS parentId, " +
            "_content AS content, _comment._gmt_create AS gmtCreate, _avatar_url AS avatarUrl, _nickname AS nickname," +
            "_comment._is_available AS Available, _comment._is_audit AS Audit FROM _comment LEFT JOIN _user ON " +
            "_user._stu_id = _comment._promulgator_id WHERE _comment._is_available = 1 AND _comment._is_audit = " +
            "1 AND _id = #{id}")
    ...
    @Update("UPDATE _comment SET _is_available = #{comment.Available}, _is_audit = #{comment.Audit} WHERE " +
            "_id = #{comment.id}")
